        ...
        while ((ch = fgetc(log_file)) != WEOF) {
            ...
            if (ch == '\n') {
                if (!skip_line) {
                    ...
                    else if (line.rfind("[Client thread/INFO]: [CHAT] Error: ", 0) == 0) {
                        line.erase(0, 36);
...
int main() {
    regex KILL_REGEX = regex("([a-zA-Z0-9_]{3,16}) wurde von ([a-zA-Z0-9_]{3,16}) get.tet");
    regex WIN_REGEX = regex("([a-zA-Z0-9_]{3,16}) hat das Spiel gewonnen!");
    regex STATUS_REGEX = regex(R"(Could not find a player by the name: -r-(((start)-([a-zA-Z0-9]{1,16})-([0-9]))|(stop)))");
